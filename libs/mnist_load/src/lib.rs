...
pub fn load_mnist_from_bytes(
    train_data: &[u8],
    train_labels: &[u8],
    test_data: &[u8],
    test_labels: &[u8],
) -> Result<MnistDataset, std::io::Error> {
    let data_processing =
        |data: &[u8], labels: &[u8]| -> std::result::Result<Vec<MnistImage>, std::io::Error> {
            let image_data = MnistFileData::new_from_bytes(data)?;
            let label_data = MnistFileData::new_from_bytes(labels)?;

            assert_eq!(
                image_data.sizes[0], label_data.sizes[0],
                "Image and label data sizes are not equal"
            );

            let image_length = (image_data.sizes[1] * image_data.sizes[2]) as usize;

            let out: Vec<MnistImage> = image_data
                .data
                .windows(image_length)
                .zip(label_data.data)
                .map(|(mnist_img, mnist_label)| MnistImage {
                    data: mnist_img
                        .to_vec()
                        .into_iter()
